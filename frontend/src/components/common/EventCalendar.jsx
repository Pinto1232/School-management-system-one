...
const EventsCalendar = ({ eventsCalendar, onEventClick, handleEventClick }) => {
  ...
  const handleDayClick = (day) => {
    ...
    const event = eventsCalendar.find(
      (e) => e.date === day.toLocaleDateString()
    );
    ...
    if (event) {
      const popover = (
        <Popover>
          ...
          <PopoverBody>Date: {event.date}</PopoverBody>
        </Popover>
      );
  ...
  const renderDays = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      days.push(
        <Popover key={`day-${i}`}>
          <PopoverTrigger>
            <GridItem p={2} borderRadius="md" {...dayStyle}>
              {i}
            </GridItem>
          </PopoverTrigger>
          {eventsForDay.length > 0 && (
            <PopoverContent>
              <PopoverHeader>
                {i} {getMonthName(currentMonth)} {currentYear}
              </PopoverHeader>
              <PopoverBody>
    ...
    const gridTemplateColumns = "repeat(7, minmax(0, " + 1 / 7 + "fr))";
