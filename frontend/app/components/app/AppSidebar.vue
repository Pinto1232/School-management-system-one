...
<template>
  ...
  >
    <div :class="['flex min-h-[88px] items-center justify-between gap-2 px-5', collapsed && 'lg:px-2']">
      <NuxtLink
        :class="[
        ...
        @click="close"
      >
        <BrandLogo :class="collapsed ? 'lg:size-8 lg:[&_img]:top-[-19px] lg:[&_img]:left-[-29px] lg:[&_img]:h-[88px] lg:[&_img]:w-[88px]' : ''" />
        <span :class="['overflow-hidden text-ellipsis whitespace-nowrap', collapsed && 'lg:hidden']">{{ brandName }}</span>
      </NuxtLink>

      <button
        class="hidden size-9 shrink-0 place-items-center rounded-full border border-line bg-surface text-ink-soft transition duration-200 hover:border-brand-400 hover:bg-brand-50 hover:text-brand-700 active:translate-y-px lg:grid"
        type="button"
    ...
    <div :class="['mt-auto grid gap-1.5 border-t border-line px-3 pt-3 pb-4', collapsed && 'lg:px-2']">
